...
makeTamatizPlot = function(res,pairs){
  ...
  plot(c(5,1),c(7,1),type='n',xaxt='n',yaxt='n',xlab='',ylab='', bty='n',ylim=c(7,0.9),xlim=c(5.2,0.9))
  for(j in 1:6){
    for(i in 1:4){
  ...
  text(rep(5.1,6),(1:6)+0.5,paste("S",rep(1:3,2),sep=''))
  text(rep(5.1,6),(1:6)+0.5,paste("S",rep(1:3,2),sep=''))
  text(rep(5.3,2),c(2.5,5.5),c("Week1","Week4") ,srt=90)
  abline(h=3.95)
...
for(colourID in colourNumbers){
  ...
  for(week in unique(d$week)){
    ...
    for(session in 1:3){
      dxx = dx[dx$session==session & dx$director==dx$speaker,] 
