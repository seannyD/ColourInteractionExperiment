...
For each annotation in the colour tiers.
	...
	For each of these signs:
		...
		Get the origin of the sign: 
			Look for annotations in the sign origin tier (named e.g. Sign_Orign_1, Sign_Orign_2) that occur within the times of the sign annotation (+ or - 50 milliseconds).
		
		Get the try marking value:
			Find annotations in the try marker tier (named "Try-marker_1") that occur in the times of the sign annotation (+ or - 50 milliseconds).
			If there is an annotation, and it is either "Yes" or blank (""), then *this sign is try-marked*
		...
		Get the teaching value:
			...
			If there is an annotation, and the annotation label is either "Yes" or blank (""), then *this is a teaching context*
